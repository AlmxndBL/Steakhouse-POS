...
# Flet 0.28+ renamed ElevatedButton to Button; keep the classroom project's
# existing UI modules compatible with both supported API shapes.
...
STATUS_REGISTRY = {
    "VACANT": {"text": "โต๊ะว่าง", "color": ThemeColors.STATUS_AVAILABLE, "icon": ft.Icons.CHECK_CIRCLE_ROUNDED},
    "OCCUPIED": {"text": "มีลูกค้า", "color": ThemeColors.STATUS_OCCUPIED, "icon": ft.Icons.RESTAURANT_ROUNDED},
    "PENDING": {"text": "รอทำ", "color": ThemeColors.STATUS_OCCUPIED, "icon": ft.Icons.HOURGLASS_TOP_ROUNDED},
    "COOKING": {"text": "กำลังปรุง", "color": ThemeColors.STATUS_BILLED, "icon": ft.Icons.LOCAL_FIRE_DEPARTMENT_ROUNDED},
    "SERVED": {"text": "พร้อมเสิร์ฟ", "color": ThemeColors.STATUS_AVAILABLE, "icon": ft.Icons.CHECK_ROUNDED},
    "PAID": {"text": "ชำระแล้ว", "color": ThemeColors.STATUS_BILLED, "icon": ft.Icons.PAYMENTS_ROUNDED},
    "CANCELLED": {"text": "ยกเลิก", "color": ThemeColors.STATUS_CANCELLED, "icon": ft.Icons.CANCEL_ROUNDED},
}


def get_status_meta(status: object) -> dict:
    """Return one consistent visual representation for a domain status."""
    key = getattr(status, "value", status)
    return STATUS_REGISTRY.get(str(key), {
        "text": str(key),
