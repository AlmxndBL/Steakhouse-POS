...
class PosMainView(ft.View):
    def __init__(self, page: ft.Page):
        ...
        self.popular_row = ft.Row(scroll=ft.ScrollMode.AUTO, spacing=8)
        self.search_input = ft.TextField(
            label="ค้นหาเมนูด้วยชื่อหรือรหัส",
            prefix_icon=ft.Icons.SEARCH_ROUNDED,
            dense=True,
            expand=True,
            on_change=lambda e: self._filter_menu(self.selected_category_id)
        )
        ...
        left_layout = ft.Column(
            ...
            spacing=0,
            controls=[
                ft.Container(content=self.search_input, padding=ft.Padding.only(left=16, right=16, top=12)),
                ft.Container(
                    content=ft.Row([ft.Text("ขายดี", weight=ft.FontWeight.BOLD, color=ThemeColors.AMBER_DARK), self.popular_row], spacing=12),
    ...
    def _load_categories_and_menu(self):
        ...
        try:
            ...
            popular_items = MenuService.get_popular_menu_items(db, limit=5)
            self.popular_row.controls.clear()
            for item in popular_items:
                self.popular_row.controls.append(ft.TextButton(
                    f"{item.code} · {item.name}",
                    on_click=lambda e, menu_item=item: self._on_menu_item_click(menu_item),
                ))
    ...
    def _filter_menu(self, category_id):
        ...
        db = SessionLocal()
        try:
            items = MenuService.search_menu_items(db, self.search_input.value or "", category_id=category_id)
            ...
            try:
                ...
                logging.debug("POS cart update skipped: %s", err)
        finally:
            db.close()
        ...
        try:
            if "สเต๊ก" in item.name:
                ...
                quantity = ft.Dropdown(
                    label="จำนวน",
                    value="1",
                    options=[ft.dropdown.Option(str(value)) for value in (1, 2, 3, 4, 5)],
                    width=360,
                )

                def add_with_options(e):
                    options_json = json.dumps({"doneness": sel_doneness.value, "sauce": sel_sauce.value}, ensure_ascii=False)
                    self.last_modifier_options[item.id] = options_json
                    self._close_dialog(dialog)
                    self._add_to_cart(item.id, qty=int(quantity.value), options_json=options_json)

                def repeat_last_modifier(e):
                    self._close_dialog(dialog)
                    self._add_to_cart(item.id, qty=int(quantity.value), options_json=self.last_modifier_options[item.id])

                repeat_button = ft.TextButton(
                    "ใช้ตัวเลือกเดิม",
                    on_click=repeat_last_modifier,
                    disabled=item.id not in self.last_modifier_options,
                )
                ...
                dialog = ft.AlertDialog(
                    ...
                    content=ft.Container(
                        width=380,
                        content=ft.Column([sel_doneness, sel_sauce, quantity], tight=True, spacing=12)
                    ),
                    actions=[
    ...
    def _add_to_cart(self, menu_item_id: int, qty: int = 1, options_json: str = None):
        if not self.order_id:
            return
        db = SessionLocal()
        try:
            OrderService.add_item_to_order(db, self.order_id, menu_item_id, qty=qty, options_json=options_json)
            self._load_cart()
        finally:
            db.close()
    ...
    def _load_cart(self):
        ...
        try:
            ...
            try:
                ...
                logging.debug("POS UI update skipped: %s", err)
        finally:
            db.close()
        ...
        try:
            ...
            if not order or not order.items or len(order.items) == 0:
                ...
                try:
                    ...
                    logging.debug("POS snackbar update skipped: %s", err)
                return
            
            sent_count = OrderService.send_new_items_to_kitchen(db, self.order_id, user_id=self.page_ref.session.store.get("user_id"))
            if sent_count == 0:
                self.page_ref.snack_bar = ft.SnackBar(ft.Text("รายการนี้ถูกส่งเข้าครัวแล้ว"), bgcolor=ThemeColors.AMBER_DARK, open=True)
                self.page_ref.update()
                return
            ...
            self.page_ref.snack_bar = ft.SnackBar(
                ft.Text(f"ส่งรายการใหม่ {sent_count} รายการของ #{order.order_number} เข้าครัวเรียบร้อยแล้ว"),
        ...
        try:
            ...
            if not order or not order.items or len(order.items) == 0:
                ...
                try:
                    ...
                    logging.debug("POS checkout notice update skipped: %s", err)
                return
            ...
            def recalc_modal(e_change):
                ...
                try:
                    ...
                    logging.debug("POS checkout dialog update skipped: %s", err)

            discount_input.on_change = recalc_modal

            def apply_discount_preset(amount: int):
                discount_input.value = str(amount)
                recalc_modal(None)
            ...
            def do_checkout_confirm(e_click):
                ...
                if not d_res.is_valid:
                    ...
                    try:
                        ...
                        logging.debug("POS discount validation refresh skipped: %s", err)
                    return
                    ...
                    content=ft.Column(
                        ...
                        controls=[
                            ...
                            ft.Row(
                                spacing=6,
                                controls=[
                                    ft.Text("Preset:", size=11, color=ThemeColors.TEXT_MUTED),
                                    *[ft.TextButton(f"{amount} ฿", on_click=lambda e, value=amount: apply_discount_preset(value)) for amount in (100, 300, 500)],
                                ],
                            ),
    ...
    def _execute_checkout(self, payment_method: str, discount_amount: float):
        ...
        try:
            ...
            try:
                ...
                logging.debug("POS checkout error display skipped: %s", err)
        finally:
            db.close()
    ...
    def _update_ui(self):
        try:
            ...
            logging.debug("POS view update skipped: %s", err)
        try:
            self.page_ref.update()
        except RuntimeError as err:
            logging.debug("POS page update skipped: %s", err)
