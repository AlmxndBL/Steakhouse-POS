    ...
    def _open_dialog(self, dialog: ft.AlertDialog):
        try:
            ...
            try:
                ...
                logging.debug("Settings dialog update skipped: %s", err)
        ...
        try:
            ...
            if dialog is not None:
                ...
                try:
                    ...
                    logging.debug("Settings page update skipped: %s", err)

    def _update_ui(self):
        try:
            self.update()
        except RuntimeError as err:
            logging.debug("Settings view update skipped: %s", err)
        try:
            self.page_ref.update()
        except RuntimeError as err:
            logging.debug("Settings page refresh skipped: %s", err)
