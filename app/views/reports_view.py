...
class ReportsView(ft.View):
                    ...
                    cells=[
                        ...
                        ft.DataCell(
                            ft.IconButton(
                                ...
                                tooltip="คืนเงิน (ต้องมี approval)",
                                visible=o.status == OrderStatus.PAID and self.page_ref.session.store.get("user_role") in {"OWNER", "MANAGER"},
                                on_click=lambda e, oid=order_id: self._open_refund_dialog(oid),
                            )
                        ),
                    ]
                )
    ...
    def _open_refund_dialog(self, order_id: int):
        reason = ft.TextField(label="เหตุผลการคืนเงิน", multiline=True, min_lines=2)
        dialog = ft.AlertDialog(
            modal=True,
            title=ft.Text("ยืนยันการคืนเงิน"),
            content=reason,
            actions=[
                ft.TextButton("ยกเลิก", on_click=lambda _: self._close_refund_dialog(dialog)),
                create_button("คืนเงิน", bg_color=ThemeColors.CRIMSON, on_click=lambda _: self._submit_refund(dialog, order_id, reason)),
            ],
        )
    ...
    def _submit_refund(self, dialog, order_id: int, reason):
        db = SessionLocal()
        try:
            OrderService.refund_order(
                ...
                approver_role=self.page_ref.session.store.get("user_role"),
            )
            self._close_refund_dialog(dialog)
            self._load_reports_data()
        except (ValueError, PermissionError) as err:
            self.page_ref.snack_bar = ft.SnackBar(ft.Text(str(err)), open=True)
            self.page_ref.update()
        finally:
            db.close()

    def _close_refund_dialog(self, dialog):
        dialog.open = False
        self.page_ref.update()
        ...
        try:
            ...
            def copy_to_clipboard(e_copy):
                try:
                    ...
                    logging.debug("Report clipboard copy skipped: %s", err)
                btn_copy.text = "คัดลอกสำเร็จแล้ว!"
                btn_copy.icon = ft.Icons.CHECK_ROUNDED
                try:
                    btn_copy.update()
                except RuntimeError as err:
                    logging.debug("Report copy button update skipped: %s", err)
                ...
                try:
                    ...
                    logging.debug("Report copy feedback update skipped: %s", err)
            ...
            def trigger_download(e_down):
                try:
                    ...
                    logging.debug("Report download launch skipped: %s", err)
    ...
    def _update_ui(self):
        try:
            ...
            logging.debug("Reports view update skipped: %s", err)
        try:
            self.page_ref.update()
        except RuntimeError as err:
            logging.debug("Reports page update skipped: %s", err)
