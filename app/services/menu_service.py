...
from database.models import Category, MenuItem, Order, OrderItem, OrderStatus, RecipeBOM
from utils.validators import Validator

class MenuService:
    @staticmethod
    def get_popular_menu_items(db: Session, limit: int = 5) -> List[MenuItem]:
        """Return active items ranked by quantity sold in paid orders."""
        safe_limit = max(1, min(int(limit), 20))
        rows = db.query(MenuItem).join(OrderItem, OrderItem.menu_item_id == MenuItem.id).join(
            Order, Order.id == OrderItem.order_id
        ).filter(
            MenuItem.is_active == True,
            Order.status == OrderStatus.PAID,
        ).group_by(MenuItem.id).order_by(func.sum(OrderItem.quantity).desc(), MenuItem.id.asc()).limit(safe_limit).all()
        if rows:
            return rows
        return db.query(MenuItem).filter(MenuItem.is_active == True).order_by(MenuItem.id.asc()).limit(safe_limit).all()
    ...
    @staticmethod
    def search_menu_items(db: Session, query_text: str, category_id: Optional[int] = None) -> List[MenuItem]:
        """Search active menu items by name or code for fast POS lookup."""
        ...
        if text:
            pattern = f"%{text}%"
            query = query.filter((MenuItem.name.ilike(pattern)) | (MenuItem.code.ilike(pattern)))
        return query.order_by(MenuItem.id.asc()).all()
