...
class BOMEngine:
    ...
    def deduct_stock_for_order(db: Session, order: Order, user_id: int) -> bool:
        ...
        for item in order.items:
            ...
            for bom in boms:
                needed_qty = bom.quantity_required * item.quantity
                ...
                if remaining_to_deduct > 0:
                    ...
                    if latest_lot:
                        ...
                        # Optional: Create an audit log for overdraft
                        audit_overdraft = AuditLog(
                            user_id=user_id,
                            action="STOCK_OVERDRAFT",
                            target_type="Ingredient",
                            target_id=ingredient_id,
                            details_json=f'{{"order": "{order.order_number}", "overdraft_qty": {remaining_to_deduct}}}'
                        )
                        db.add(audit_overdraft)
                    else:
                        # No lots at all, we can't deduct. In a real system, maybe create a dummy lot.
                        pass

                # Update average ingredient cost
                ingredient = db.query(Ingredient).filter(Ingredient.id == ingredient_id).first()
                if ingredient and remaining_to_deduct > 0:
                    # Warn: Not enough stock in active lots! Still record deduction
                    pass
